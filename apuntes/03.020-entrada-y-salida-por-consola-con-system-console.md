...
```
¿Necesito leer una línea de texto desde la consola?
    └─► `Console.ReadLine()`

¿Necesito leer una tecla sin esperar Enter?
    └─► `Console.ReadKey()`

¿Necesito escribir sin salto de línea?
    └─► `Console.Write(...)`

¿Necesito escribir con salto de línea?
    └─► `Console.WriteLine(...)`

¿Necesito limpiar la pantalla o cambiar colores?
    └─► `Console.Clear()`, `Console.ForegroundColor`, `Console.BackgroundColor`, `Console.ResetColor()`

¿Necesito mover o mostrar el cursor?
    └─► `Console.SetCursorPosition(...)`, `Console.GetCursorPosition()`, `Console.CursorVisible`

¿Necesito trabajar con entrada, salida y error por separado?
    └─► `Console.In`, `Console.Out`, `Console.Error`

¿Necesito redirigir datos desde o hacia archivos?
    └─► operadores de shell: `>`, `>>`, `2>`, `<`, `|`

¿Necesito capturar texto desde un archivo o enviar salida a otro programa?
    └─► redirección de la consola y `Console.SetOut(...)`

¿Necesito recordar la idea principal?
    └─► `Console` sirve para interactuar con el usuario y con el entorno de ejecución

Ejemplos clave:
    `Console.ReadLine()`   → leer texto
    `Console.ReadKey()`    → leer una tecla
    `Console.WriteLine()`  → mostrar texto
    `Console.In/Out/Error` → flujos estándar
    `>` / `<` / `|`        → redirección y canalización
    `Console.ForegroundColor` → apariencia
```
