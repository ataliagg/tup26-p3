...
También se puede usar del lado izquierdo de una asignación: si la referencia es `null`, la asignación simplemente no se ejecuta.

```csharp
Cliente? cliente = ObtenerCliente();
cliente?.Nombre = "Ana";   // sólo asigna si cliente no es null

Pedido? pedido = ObtenerPedido();
pedido?.Lineas?[0] = nuevaLinea;   // también funciona con indexadores condicionales
```

También se usa mucho para invocar delegados o funciones de forma segura, sin tener que comprobar explícitamente si son `null`:

```csharp
Action? notificar = mensaje => Console.WriteLine(mensaje);
notificar?.Invoke("Pedido procesado");   // sólo ejecuta si no es null

Func<int, int, int>? sumar = (a, b) => a + b;
int? resultado = sumar?.Invoke(2, 3);     // 5 si existe, null si sumar es null
```
...
¿Quiero asignar sólo si la referencia no es null?
    └─► obj?.Miembro = valor      (no hace nada si obj es null)

¿Quiero invocar un delegado de forma segura?
    └─► delegado?.Invoke(args)    (no hace nada si el delegado es null)
