...
class Interprete {
    ...
    public Interprete(List<Token> tokens) => this.tokens = tokens;

    // Gracias al token Fin, Actual nunca es null
    private Token Actual     => this.tokens[this.pos];
    private Token Consumir() => this.tokens[this.pos++];
...
class Contexto {
    ...
    public void Asignar(string nombre, int valor) => this.vars[nombre] = valor;

    public int Obtener(string nombre) =>
        this.vars.TryGetValue(nombre, out int v)
            ? v
            : throw new InvalidOperationException($"Variable '{nombre}' no definida.");

    public void Mostrar() {
        if (this.vars.Count == 0) { Console.WriteLine("  (sin variables)"); return; }
        foreach (var (k, v) in this.vars) Console.WriteLine($"  {k} = {v}");
    }
}
```
...
class Contexto {
    private readonly Dictionary<string, int> vars = new();
    public void Asignar(string n, int v) => this.vars[n] = v;
    public int Obtener(string n) =>
        this.vars.TryGetValue(n, out int v) ? v
