...
```
¿Necesito construir una CLI simple pero real?
    └─► un programa de consola con entrada, parseo, lógica y salida

¿Necesito modelar configuración inmutable?
    └─► `record`

¿Necesito parsear argumentos de línea de comandos?
    └─► recorrer `args` y consumir opciones con un índice

¿Necesito leer desde archivo o desde stdin?
    └─► `File.ReadAllText(...)` o `Console.In.ReadToEnd()`

¿Necesito escribir a archivo o a stdout?
    └─► `File.WriteAllText(...)` o `Console.Write(...)`

¿Necesito procesar CSV?
    └─► separar encabezados, filas y valores con diccionarios

¿Necesito validar columnas antes de sumar?
    └─► verificar que existan antes de recorrer los datos

¿Necesito alinear un reporte de texto?
    └─► medir anchos primero, después usar `PadRight` y `PadLeft`

¿Necesito mostrar ayuda clara?
    └─► `PrintHelp()` con usos, opciones y ejemplos

¿Necesito que el programa sea reutilizable desde cualquier carpeta?
    └─► publicarlo como Global Tool

Ejemplos clave:
    `record`         → configuración inmutable
    `switch`         → parseo de opciones
    `TryParse`       → validación numérica segura
    `Dictionary`     → filas del CSV por nombre de columna
    `PadRight/Left`  → salida tabular alineada
    `Console.Error`  → mensajes de error separados del output
    `dotnet tool`    → distribución de la CLI
```
