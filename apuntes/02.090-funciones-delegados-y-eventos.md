...
```csharp
static string Formatear(string texto, bool mayusculas = false, bool trim = true, int maxLongitud = 100) {
    if (trim)        
        texto = texto.Trim();
    if (mayusculas)  
        texto = texto.ToUpper();
...
static double Area(double base, double alto) 
    => base * alto / 2;
...
public class NotificadorEmail {
    public void Suscribir(SistemaPedidos sistema) => sistema.PedidoCreado += OnPedidoCreado;
...
public class SistemaAuditoria {
    public void Suscribir(SistemaPedidos sistema) => sistema.PedidoCreado += (_, e) =>
...
public class PipelineBuilder<T> {
    ...
    public IEnumerable<T> Ejecutar(IEnumerable<T> fuente) => this.pasos.Aggregate(fuente, (datos, paso) => paso(datos));
}
